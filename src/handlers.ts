...
import { ISteelScraperService } from "./steel-api.js";
import { formatSuccessResponse, formatErrorResponse } from "./response-formatter";
import { SteelErrorCode } from "./errors.js";

export function setupHandlers(server: Server, steelScraperService: ISteelScraperService) {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "visit_with_browser") {
      try {
        ...
        const result = await steelScraperService.scrapeWithBrowser({
          url,
          format,
          screenshot,
          pdf,
