...
export interface ISteelScraperService {
  scrapeWithBrowser(options: ScrapeOptions): Promise<ScrapeResult>;
  healthCheck(): Promise<boolean>;
  getInfo(): Promise<any>;
}

export class SteelDevScraperService implements ISteelScraperService {
  ...
  async scrapeWithBrowser(options: ScrapeOptions): Promise<ScrapeResult> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let errorCode: SteelErrorCode;
        if (response.status >= 500) {
          errorCode = SteelErrorCode.SERVER_ERROR;
        } else if (response.status === 429) {
          errorCode = SteelErrorCode.RATE_LIMIT_EXCEEDED;
        } else if (response.status >= 400) {
          errorCode = SteelErrorCode.CLIENT_ERROR;
        } else {
      ...
      if (error instanceof Error) {
        ...
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
          errorCode = SteelErrorCode.NETWORK_UNAVAILABLE;
        } else if (error.name === 'TimeoutError' || error.message.includes('timeout')) {
          errorCode = SteelErrorCode.NETWORK_TIMEOUT;
        } else {
          errorCode = SteelErrorCode.UNKNOWN_ERROR;
        }
        
        errorMetadata.originalError = {
          name: error.name,
  ...
  async getInfo(): Promise<any> {
    try {
      ...
      const steelError = createSteelError(
        SteelErrorCode.SERVER_ERROR,
...
// For backward compatibility, keep the original SteelAPI class
